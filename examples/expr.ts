...
// this constructor combines two Expr<B> into an encoded Expr<boolean> representing their equality
function Equal_y<B>(eb1: Expr<B>, eb2: Expr<B>) {
    return <R>(eq:(_: boolean) => R): Expr<R> => ({
        equal: cont => cont({
          eb1,
          eb2,
...
function fmap<A, B>(f: ( _: A) => B, ea: Expr<A>): Expr<B> {
    switch(ea._tag) {
        ...
        case "Eq": {
            return ea.equal(ea => Equal_y(ea.eb1, ea.eb2)((_) => f(ea.eq(_))))
        }
    }
}
...
function evalExpr<A>(ea: Expr<A>): A {
  switch(ea._tag) {
        ...
        case "Eq": {
          return ea.equal(ea => ea.eq(evalExpr(ea.eb1) === evalExpr(ea.eb2)))
        }
    }
}
